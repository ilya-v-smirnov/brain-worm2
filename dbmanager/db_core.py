...
def get_project_home_dir() -> Path:
    ...
    (работает корректно на Windows, macOS и Linux).
    """
...
def init_db_schema(conn: Optional[sqlite3.Connection] = None) -> None:
    ...
    Столбцы lecture_text_path / lecture_audio_path сохранены в схеме для
    обратной совместимости со старыми БД. В новой версии приложения они
    не используются, но удалять их из схемы было бы преждевременно.
    """
    own_conn = False
    if conn is None:
        conn = get_connection()
